...
struct Red_Black_Tree {
  ...
  void rotate( node *x, bool b ) {
    ...
    else x->parent->ch[ !( x == x->parent->ch[0] ) ] = y;
  ...
  key_type rb_find_kth( int k ) {
    ...
    node *x = root;
    while ( 1 ) {
      int lo_rank = x->ch[0]->size + 1,
          hi_rank = x->ch[0]->size + x->cnt;
      if ( lo_rank <= k && k <= hi_rank ) return x->key;
      if ( k < lo_rank ) x = x->ch[0];
	  else k -= hi_rank, x = x->ch[1];
    }
  }
  ...
  node* rb_erase( key_type key ) {
    ...
    node *y = ( z->ch[0] == null || z->ch[1] == null ) ?
              z : adjacent( z->key, 1 );
